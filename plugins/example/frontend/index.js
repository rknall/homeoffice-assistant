...
function ExamplePage() {
  ...
  return h(
    ...
    h(
      'form',
      { onSubmit: createNote, className: 'bg-white p-4 rounded-lg shadow mb-6' },
    ...
    h(
      ...
      { className: 'space-y-4' },
      notes.length === 0
        ? h('p', { className: 'text-gray-500' }, 'No notes yet. Create one above!')
        : notes.map((note) =>
            h(
              'div',
              ...
              note.content && h('p', { className: 'text-gray-600 mt-2' }, note.content),
              h(
                'p',
...
function DashboardWidget() {
  ...
  return h(
    ...
    h('p', { className: 'text-sm text-gray-500 mt-2' }, `${info.note_count} notes`)
  )
}
