...
function TimeTrackingPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const bal = await apiGet(`/leave-balance?year=${year}&company_id=${selectedCompanyId}`)
      setBalance(bal)
    } catch (e) {
      setError(e.message)
    } finally {
  ...
  if (loading) {
    return h('div', { className: 'p-6' }, h('p', { className: 'text-gray-500' }, 'Loading...'))
  }
  ...
  return h(
    ...
    // Error message
    error && h('div', { className: 'bg-red-100 text-red-700 p-4 rounded mb-4' }, error),
          ...
          h(
            ...
            checkInStatus?.has_open_entry
              ? h(
                  ...
                  getElapsedTime() &&
                    h('span', { className: 'text-gray-400 ml-1' }, `(${getElapsedTime()})`),
                )
            ...
            weekRecords.forEach((record) => {
              if (record.entries && record.entries.length > 0) {
                ...
                tableRows.push({
                  ...
                  gross_minutes: record.gross_hours ? Math.round(record.gross_hours * 60) : null,
                })
              }
            })
            return h(
                    ...
                    : tableRows.map((row, i) =>
                        h(
                          'tr',
                          {
                            ...
                            onClick: () => {
                              const [year, month, day] = row.record.date.split('-').map(Number)
                          ...
                          h(
                            ...
                            row.isFirstOfDay ? getCompanyName(row.record.company_id) : '',
                          ),
                          ...
                          // Check In
                          h('td', { className: 'px-4 py-2' }, formatTime(row.check_in)),
                          // Check Out
                          h(
                            'td',
                            { className: 'px-4 py-2' },
                      ...
                      h(
                        ...
                        h(
                          ...
                          h(
                            'span',
                            { className: 'font-medium' },
                            dayRecord.break_minutes ? `${dayRecord.break_minutes} min` : '-',
                          ),
                        ),
                        h(
                          'div',
                          ...
                          h(
                            ...
                            dayRecord.net_hours ? `${dayRecord.net_hours.toFixed(1)}h` : '-',
                          ),
                        ),
                      ),
                    ),
                  ...
                  dayRecord.work_location &&
                    h(
                      ...
                      h(
                        'span',
                        { className: 'font-medium' },
                        WORK_LOCATIONS[dayRecord.work_location] || dayRecord.work_location,
                      ),
                    ),
                  // Notes
                  dayRecord.notes &&
...
function CompanyTimeSettingsWidget({ companyId }) {
  ...
  if (loading) {
    return h('div', { className: 'p-4 text-gray-500' }, 'Loading time settings...')
  }
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h('span', { className: 'font-medium' }, `${settings.daily_overtime_threshold}h`),
      ),
      h(
        'div',
        null,
        h('span', { className: 'text-gray-500' }, 'Weekly OT Threshold: '),
        h('span', { className: 'font-medium' }, `${settings.weekly_overtime_threshold}h`),
      ),
      h(
        'div',
        null,
        h('span', { className: 'text-gray-500' }, 'Lock Period: '),
        h('span', { className: 'font-medium' }, `${settings.lock_period_days} days`),
      ),
    ),
  )
}
