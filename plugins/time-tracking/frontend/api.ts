...
export const timeRecordsApi = {
  list: (params?: {
    ...
    return request<TimeRecordListResponse>(`/records${query ? `?${query}` : ''}`)
  },
  ...
  update: (recordId: Uuid, data: TimeRecordUpdate): Promise<TimeRecordWithWarnings> => {
...
export const allocationsApi = {
  ...
  create: (recordId: Uuid, data: TimeAllocationCreate): Promise<TimeAllocation> => {
    return request<TimeAllocation>(`/records/${recordId}/allocations`, {
      method: 'POST',
      body: JSON.stringify(data),
    })
  ...
  update: (
    ...
    return request<TimeAllocation>(`/records/${recordId}/allocations/${allocationId}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
  },
...
export const companySettingsApi = {
  get: (companyId: Uuid): Promise<CompanyTimeSettings | null> => {
    return request<CompanyTimeSettings | null>(`/settings/company/${companyId}`)
  },

  create: (companyId: Uuid, data: CompanyTimeSettingsCreate): Promise<CompanyTimeSettings> => {
  ...
  update: (companyId: Uuid, data: CompanyTimeSettingsUpdate): Promise<CompanyTimeSettings> => {
    return request<CompanyTimeSettings>(`/settings/company/${companyId}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    })
...
export const reportsApi = {
  getMonthly: (companyId: Uuid, year: number, month: number): Promise<MonthlyReportSummary> => {
  ...
  getWeek: (companyId: Uuid, startDate: string): Promise<WeekData> => {
    return request<WeekData>(`/reports/week?company_id=${companyId}&start_date=${startDate}`)
  },
}
