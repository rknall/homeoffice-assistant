...
def user_has_permission(db: Session, user: User, permission_code: str, company_id: uuid.UUID | None = None) -> bool:
...
def get_user_permissions(db: Session, user: User, company_id: uuid.UUID | None = None) -> set[str]:
    ...
    global_roles = db.query(UserRole).filter(UserRole.user_id == user.id, UserRole.company_id == None).all()
    ...
    if company_id:
        company_roles = db.query(UserRole).filter(UserRole.user_id == user.id, UserRole.company_id == company_id).all()
...
def get_user_roles(db: Session, user: User, company_id: uuid.UUID | None = None) -> list[Role]:
    ...
    if company_id:
        user_roles_query = user_roles_query.filter(sa.or_(UserRole.company_id == None, UserRole.company_id == company_id))
    else:
        user_roles_query = user_roles_query.filter(UserRole.company_id == None)
...
def assign_role_to_user(db: Session, user_id: uuid.UUID, role_id: uuid.UUID, company_id: uuid.UUID | None = None, assigned_by: User | None = None) -> UserRole:
    user_role = UserRole(
        user_id=user_id,
        role_id=role_id,
...
def remove_role_from_user(db: Session, user_id: uuid.UUID, role_id: uuid.UUID, company_id: uuid.UUID | None = None) -> bool:
...
def register_permission(db: Session, code: str, module: str, description: str | None = None) -> Permission:
